...
impl StaticObject
{
                        ...
                        faces: &Vec<StaticObjectFace>) -> Vec<StaticObjectSubmesh>
    {
        ...
        for (submesh_name, faces) in submesh_map
        {
            ...
            let mut submesh_vertices: Vec<StaticObjectVertex> = Vec::with_capacity(vertex_count as usize);
            for i in min_vertex..vertex_count
            {
                ...
                if has_vertex_colors
                {
                    submesh_vertices.push(StaticObjectVertex::new_color(vertices[i as usize], uv,
            ...
            submeshes.push(StaticObjectSubmesh::new(submesh_name.clone(), submesh_vertices, indices));
        }

        return submeshes;
    }
    fn create_submesh_map(faces: &Vec<StaticObjectFace>) -> HashMap<String, Vec<&StaticObjectFace>>
    {
...
impl StaticObjectSubmesh
{
    ...
    pub fn vertices(&mut self) -> &mut Vec<StaticObjectVertex> { &mut self.vertices }
    pub fn indices(&mut self) -> &mut Vec<u32> { &mut self.indices }
}
