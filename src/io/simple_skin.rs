...
impl SimpleSkin
{
    ...
    fn read<T: Read + Seek>(reader: &mut BinaryReader<T>) -> io::Result<Self>
    {
        ...
        let bounding_box = if major == 4 { Box3D::read(reader)? } else { Box3D::zero() };
        let bounding_sphere = if major == 4 { Sphere::read(reader)? } else { Sphere::zero() };

        if (vertex_type == 0 && vertex_size != 52) ||
            (vertex_type == 1 && vertex_size != 56)
        {
        ...
        {
            let mut submesh_vertices: Vec<SimpleSkinVertex> = Vec::with_capacity(submesh.vertex_count as usize);
    ...
    {
        let index = self.submeshes.iter()
            .position(|submesh| submesh.name == name);

        match index
        {
            Some(x) => { self.submeshes.remove(x); }
            _ => {}
        }
    }
    ...
    pub fn submeshes(&mut self) -> &mut Vec<SimpleSkinSubmesh> { &mut self.submeshes }
    ...
    pub fn bounding_box(&mut self) -> Box3D
    {
        ...
        {
            let mut min = Vector3::new(std::f32::INFINITY,
                                       std::f32::INFINITY,
                                       std::f32::INFINITY);
            let mut max = Vector3::new(std::f32::NEG_INFINITY,
...
impl SimpleSkinSubmesh
{
    ...
    pub fn vertices(&mut self) -> &mut Vec<SimpleSkinVertex> { &mut self.vertices }
    pub fn indices(&mut self) -> &mut Vec<u16> { &mut self.indices }
}

impl SimpleSkinVertex
{
    pub fn new_basic(position: Vector3, influences: [u8; 4], weights: [f32; 4],
                 normal: Vector3, uv: Vector2) -> Self
    {
        SimpleSkinVertex
        {
            position,
